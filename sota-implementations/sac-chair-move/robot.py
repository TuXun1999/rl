...
def add_grasp_constraint(config_dict, grasp, robot_state_client):
    ...
    if hasattr(config_dict, "vector_alignment") and config_dict["vector_alignment"] is not None:
        return
...
class SpotRLEnvChairMove(EnvBase):
    ...
    def _make_spec(self, td_params):
        ...
        self.observation_spec = Composite(
            ...
            hand_current_pose = Unbounded(shape = (2,), dtype=torch.float32),
        )
    ...
    def _reset(self, tensordict):
        ...
        hand_current_pose = torch.tensor([hx, hy], device=self.device)
        out = TensorDict(
            {
