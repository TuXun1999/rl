...
def main(cfg: DictConfig):  # noqa: F821
    ...
    prior_model = make_prior_agent(cfg, device=device)

    # TODO: replace it with diffusion policy
    il_policy = prior_model

    ib_model = make_ib_policy_agent(il_policy, model[0], model[1], device=device)
    

    # Create TD3 loss
    loss_module, target_net_updater = make_loss_module(cfg, model, il_policy)
    ...
    collector = make_collector(
        cfg,
        train_env,
        ib_model, # Use the imitation-bootstrapped RL policy as the policy actor
            ...
            if collected_frames >= init_random_frames:
                ...
                for _ in range(num_updates):
                    ...
                    with timeit("rb - sample"):
                        ...
                        # del sampled_tensordict["state_action_value"]
        ...
        if abs(collected_frames % eval_iter) < frames_per_batch:
            with set_exploration_type(
                ...
                eval_rollout = eval_env.rollout(
                    eval_rollout_steps,
                    # Evaluate the imitation-bootstrapped RL policy on the environment
                    ib_model, #exploration_policy
    ...
    print("Number of times IL policy was chosen: ", loss_module.choose_il)
